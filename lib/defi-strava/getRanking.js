...
import { semainePrecedente } from './weekUtils';
...
// Compte combien de semaines D'AFFILÉE (en remontant à partir de la
// semaine donnée, elle incluse) le même nom a fini #1 — pour la mention
// spéciale "3e victoire de suite !" dans le résumé du lundi. Plafonné à
// 24 semaines en arrière par précaution (évite une boucle sans fin en
// cas de données incohérentes).
export async function calculerStreakHebdomadaire(nomGagnant, semaine) {
  if (!nomGagnant) return 0;
  let streak = 1;
  let semaineCourante = semaine;

  for (let i = 0; i < 24; i++) {
    const { semaine: precedente } = semainePrecedente(semaineCourante);
    if (!precedente) break;
    const classement = await getRankingPourPeriode(precedente.debut, precedente.fin);
    if (classement[0]?.nom !== nomGagnant) break;
    streak++;
