...
async function envoyerAuxAbonnements(subs, payload) {
...
export async function envoyerPushATous(payload) {
  ...
  return envoyerAuxAbonnements(subs, payload);
}

// Envoie une notification push à un SEUL participant (tous ses appareils
// abonnés) — utilisé pour les tests, afin de ne jamais déranger les
// autres participants pendant qu'on vérifie que ça fonctionne.
export async function envoyerPushAUnParticipant(participantId, payload) {
  const supabase = getSupabaseAdmin();
