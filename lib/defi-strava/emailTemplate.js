...
function ligneClassement(row, dernier) {
  ...
  return `
    <tr>
      <td style="padding:9px 0; ${dernier ? '' : 'border-bottom:1px solid #EDEFF1;'} font-family:'Courier New',monospace; font-weight:800; color:${couleurRang(row.rang)}; width:44px;">
        ${m || `#${row.rang}`}
      </td>
      <td style="padding:9px 0; ${dernier ? '' : 'border-bottom:1px solid #EDEFF1;'} font-weight:600; color:#14213D;">
        ${row.nom}
      </td>
      <td style="padding:9px 0; ${dernier ? '' : 'border-bottom:1px solid #EDEFF1;'} text-align:right; font-family:'Courier New',monospace; font-weight:700; color:#14213D;">
        ${row.totalFormate}
      </td>
      <td style="padding:9px 0; ${dernier ? '' : 'border-bottom:1px solid #EDEFF1;'} text-align:right; font-family:'Courier New',monospace; font-size:11.5px; color:#9aa5c0; width:70px;">
...
function tableauClassement(lignes, texteVide) {
  if (!lignes || lignes.length === 0) {
    return `<p style="color:#9aa5c0; font-size:13px; margin:6px 0 0;">${texteVide}</p>`;
  }
  const corps = lignes.map((r, i) => ligneClassement(r, i === lignes.length - 1)).join('');
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse; margin-top:4px;">${corps}</table>`;
}
...
function emailShell(contenuHtml, piedDePage) {
  return `
<!DOCTYPE html>
<html>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center">
        ...
        <![endif]-->
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.08);">

    <tr><td>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0F2138; border-top:4px solid ${ORANGE_STRAVA};">
      <tr>
        <td style="padding:16px 28px;">
          <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td valign="middle" width="60" style="padding-right:14px;">
                <img src="${LOGO_DEFI_URL}" alt="Les Entreprises PEP x Strava" width="60" style="display:block;">
              </td>
              <td valign="middle">
                <div style="color:${ORANGE_STRAVA}; font-size:11px; letter-spacing:0.14em; font-weight:700; text-transform:uppercase;">Défi du mois</div>
                <div style="color:#ffffff; font-size:21px; font-weight:800; letter-spacing:-0.01em; margin-top:2px;">Défi Strava - PEPTalk</div>
              </td>
            </tr>
          </table>
        </td>
...
function boutonVoirClassement(url, texte) {
  return `
    <div style="text-align:center; margin-top:28px;">
      <a href="${url}"
         style="display:inline-block; background:${ORANGE_STRAVA}; color:#ffffff; text-decoration:none; padding:12px 26px; border-radius:8px; font-weight:700; font-size:13.5px;">
        ${texte} →
      </a>
    </div>`;
}
...
function buildEmailHtml({ semaine, semaineNumero, top3Semaine, moisLisible, classementMois, streakSemaine }) {
  ...
  const contenu = `
    <h1 style="margin:0 0 6px; font-size:20px; color:#14213D; font-weight:800;">Résumé de la Semaine ${semaineNumero}</h1>
    <p style="margin:0 0 22px; font-size:13.5px; color:#6b7480; line-height:1.5;">
    ...
    ${gagnantSemaine ? `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FFF8EE; border:1px solid rgba(252,76,2,0.25); border-radius:8px; margin-bottom:24px;">
      <tr>
        <td style="padding:14px 18px; font-size:14px; color:#14213D; line-height:1.5;">
    ...
    <div style="font-size:11px; text-transform:uppercase; letter-spacing:0.07em; font-weight:800; color:#6b7480; margin-bottom:2px;">
      🏅 La semaine passée — ${semaine}
    </div>
    ${tableauClassement(top3Semaine, "Personne n'a bougé cette semaine.")}

    <div style="height:1px; background:#EDEFF1; margin:26px 0;"></div>

    ${meneur ? `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F8FA; border:1px solid #EDEFF1; border-radius:8px; margin-bottom:16px;">
      <tr>
        <td style="padding:14px 18px; font-size:13.5px; color:#14213D;">
...
function buildNouveauMeneurHtml({ classement, ancienMeneurNom }) {
  ...
  const contenu = `
    <h1 style="margin:0 0 16px; font-size:20px; color:#14213D; font-weight:800;">🥇 Nouveau meneur du Défi Strava !</h1>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FFF8EE; border:1px solid rgba(252,76,2,0.25); border-radius:8px; margin-bottom:24px;">
      <tr>
        <td style="padding:16px 18px; font-size:14px; color:#14213D; line-height:1.5;">
          ${ancien
...
function podiumVisuel(classement) {
  ...
  const carte = (r, medaille, coul, hauteurBloc) => r ? `
    <td width="33%" valign="bottom" style="padding:0 4px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="text-align:center; padding:14px 6px 12px;">
            <div style="font-size:28px; line-height:1;">${medaille}</div>
            <div style="font-weight:800; font-size:12.5px; color:#14213D; margin-top:6px;">${r.nom}</div>
            <div style="font-family:'Courier New',monospace; font-weight:700; font-size:12.5px; color:#14213D; margin-top:2px;">${r.totalFormate}</div>
          </td>
        </tr>
        <tr>
          <td style="height:${hauteurBloc}px; background:${coul}22; border:1px solid ${coul}; border-radius:8px 8px 0 0;"></td>
        </tr>
      </table>
    </td>` : `<td width="33%"></td>`;
  ...
  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 22px;">
      <tr>
        ${carte(deuxieme, '🥈', ARGENT, 46)}
        ${carte(premier, '🥇', OR, 66)}
        ${carte(troisieme, '🥉', BRONZE, 32)}
      </tr>
    </table>`;
}
...
function sectionRecordsChanges(categoriesChangees) {
  if (!categoriesChangees || categoriesChangees.length === 0) {
    return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F8FA; border:1px solid #EDEFF1; border-radius:8px; margin:22px 0;">
      <tr>
        <td style="padding:14px 18px; font-size:12.5px; color:#495260;">
  ...
  const lignes = categoriesChangees.map((cat) => `
    <tr>
      <td style="padding:9px 0; border-bottom:1px solid #EDEFF1; font-size:15px; width:30px;">${cat.icone}</td>
      <td style="padding:9px 0; border-bottom:1px solid #EDEFF1;">
        <div style="font-weight:700; font-size:12.5px; color:#14213D;">${cat.titre}</div>
        <div style="font-size:11.5px; color:#6b7480;">${cat.detenteur} — <span style="font-family:'Courier New',monospace; font-weight:700; color:#14213D;">${cat.valeur}</span></div>
        ${cat.ancienDetenteur ? `<div style="font-size:10.5px; color:#9aa5c0; margin-top:1px;">auparavant : ${cat.ancienDetenteur} (${cat.ancienneValeur})</div>` : ''}
      </td>
    </tr>`).join('');

  return `
    <div style="font-size:11px; text-transform:uppercase; letter-spacing:0.07em; font-weight:800; color:#6b7480; margin:22px 0 6px;">
      🏆 Records du Hall of Fame mis à jour ce mois-ci
    </div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">${lignes}</table>`;
}
...
function buildFinDeMoisHtml({ moisLisible, moisIso, classementFinal, categoriesChangees, streakMois }) {
  ...
  const contenu = `
    <h1 style="margin:0 0 6px; font-size:20px; color:#14213D; font-weight:800;">🏆 ${capitaliser(moisLisible)} est terminé !</h1>
    <p style="margin:0 0 18px; font-size:13.5px; color:#6b7480; line-height:1.5;">
      Félicitations à tout le monde d'avoir participé ! Voici le podium final.
    </p>

    ${premier ? `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FFF8EE; border:1px solid rgba(252,76,2,0.25); border-radius:8px; margin-bottom:20px;">
      <tr>
        <td style="padding:14px 18px; font-size:14px; color:#14213D; line-height:1.5;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F8FA; border:1px solid #EDEFF1; border-radius:8px; margin-bottom:20px;">
      <tr>
        <td style="padding:12px 18px; font-size:12.5px; color:#495260;">
    ...
    ${classementFinal.length === 0 ? `<p style="color:#9aa5c0; font-size:13px;">Personne n'a bougé ce mois-ci — le prochain mois nous appartient !</p>` : ''}
