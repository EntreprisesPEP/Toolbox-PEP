...
export async function calculerHallOfFame(dateCoupureISO = null) {
  ...
  let requeteActivites = supabase
    .from('activities')
    .select('participant_id, duree_secondes, date_debut, date_debut_locale, total_photo_count');
  let requeteMeneurChangements = supabase.from('meneur_changements').select('ancien_meneur_id, nouveau_meneur_id, created_at');
  let requeteVotes = supabase.from('votes').select('votant_participant_id, vote_pour_participant_id, semaine_debut');

  if (dateCoupureISO) {
    // Filtre grossier en UTC ici (affiné ensuite en JS via la vraie date
    // locale de chaque activité, comme dans fetchActivitesEntre).
    const finInclusiveUTC = `${dateCoupureISO}T23:59:59`;
    requeteActivites = requeteActivites.lte('date_debut', finInclusiveUTC);
    requeteMeneurChangements = requeteMeneurChangements.lte('created_at', finInclusiveUTC);
    requeteVotes = requeteVotes.lte('semaine_debut', dateCoupureISO);
  }
