...
// Récupère toutes les données brutes nécessaires au Hall of Fame EN UNE
// SEULE FOIS — permet de calculer plusieurs versions (ex: "tel qu'il
// était à 2 dates différentes") sans refaire les requêtes Supabase à
// chaque fois, ce qui évite de doubler le temps d'exécution (et le
// risque de dépassement de temps sur Vercel) quand on compare 2 mois.
export async function fetchDonneesHallOfFame() {
  ...
  const [{ data: participants }, { data: activites }, { data: meneurChangements }, { data: votes }] =
    await Promise.all([
      ...
      supabase.from('meneur_changements').select('ancien_meneur_id, nouveau_meneur_id, created_at'),
      supabase.from('votes').select('votant_participant_id, vote_pour_participant_id, semaine_debut'),
    ]);
  return { participants, activites, meneurChangements, votes };
}

// Calcule le Hall of Fame à partir de données DÉJÀ récupérées (voir
// fetchDonneesHallOfFame ci-dessus) — aucune requête réseau ici, juste
// du calcul en mémoire, ce qui le rend rapide à répéter pour plusieurs
// dates de coupure.
export function calculerHallOfFameDepuisDonnees(donnees, dateCoupureISO = null) {
  const { participants, activites, meneurChangements, votes } = donnees;

  const participantsParId = {};
  (participants || []).forEach((p) => { participantsParId[p.id] = p; });

  let acts = (activites || []).map((a) => ({ ...a, eff: dateEffective(a) }));
  let meneurChangementsFiltres = meneurChangements || [];
  let votesFiltres = votes || [];

  if (dateCoupureISO) {
    acts = acts.filter((a) => toDateISO(a.eff) <= dateCoupureISO);
    const finInclusiveUTC = `${dateCoupureISO}T23:59:59`;
    meneurChangementsFiltres = meneurChangementsFiltres.filter((mc) => mc.created_at <= finInclusiveUTC);
    votesFiltres = votesFiltres.filter((v) => v.semaine_debut <= dateCoupureISO);
  }
  ...
  // ou -> la date de coupure si on recalcule "tel que c'était" à un
  // moment passé, pour "Le duel légendaire" et "No Days Off" notamment).
  const premiereDate = acts.reduce((min, a) => (a.eff < min ? a.eff : min), acts[0].eff);
  const aujourdHui = dateCoupureISO ? new Date(dateCoupureISO + 'T23:59:59') : new Date();
  ...
  for (const mc of meneurChangementsFiltres) {
...
// Version "tout-en-un" (fetch + calcul) pour les usages qui n'ont besoin
// que d'une seule date — ex: la page /defi-strava/ qui affiche le Hall
// of Fame actuel. Pour comparer 2 dates (comme le courriel de fin de
// mois), utiliser plutôt fetchDonneesHallOfFame() une seule fois, puis
// calculerHallOfFameDepuisDonnees() deux fois — beaucoup plus rapide.
export async function calculerHallOfFame(dateCoupureISO = null) {
  const donnees = await fetchDonneesHallOfFame();
  return calculerHallOfFameDepuisDonnees(donnees, dateCoupureISO);
}
