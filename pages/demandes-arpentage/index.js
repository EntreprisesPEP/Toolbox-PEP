...
function DemandeArpentageApp({ userId, nom, poste, email, accessToken }) {
  ...
  return (
    <div style={{
      ...
      <EnTeteApp
        titre="Demande d'arpentage"
        sousTitre="Formulaire de demande d'arpentage"
        mode={mode}
        onChangerMode={setMode}
        nom={nom}
        poste={poste}
        onAccueil={() => { setTab('nouvelle'); setConfirmation(null); }}
      />
...
export default function DemandesArpentagePage() {
  ...
  if (!session) {
    return <GardeConnexion appSlug="demandes-arpentage" nomApp="Demandes d'arpentage" onPret={(s) => setSession(s)} />;
  }
