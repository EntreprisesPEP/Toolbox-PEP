...
const APP_SLUG = "petits-outils";

/* ---------------------------------------------------------------------
   AUTH GATE — Demande petits outils
   Même patron que Planification hebdomadaire : login obligatoire, accès
   réservé aux personnes cochées "Demande petits outils" dans le panneau
...
export default function AuthGate({ onDone }) {
  ...
  const demanderRecuperation = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
      redirectTo: "https://www.toolbox-pep.com/petits-outils/",
    });
    setEnvoi(false);
    if (error) {
    ...
    return (
      <Cadre>
        <div style={{ textAlign: "center", fontSize: 14.5, color: "#495260", lineHeight: 1.6, marginBottom: 18 }}>
          Ton compte existe, mais tu n'as pas accès à Demande petits outils.
