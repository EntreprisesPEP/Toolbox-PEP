...
// Client sur le schema strava_challenge (pour retrouver le vrai nom via
// le courriel de connexion — pas de table de "profils" séparée, on
// réutilise directement la table participants déjà existante)
const supabaseDefi = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { db: { schema: "strava_challenge" } });

const LOGO_PEP = "/pep-logo-noir.png";
...
export default function AuthGate({ onDone }) {
  ...
  async function resoudreParticipant(session) {
    const { data: participant } = await supabaseDefi
      .from("participants").select("id, nom").eq("email", session.user.email).maybeSingle();
  ...
  useEffect(() => {
    ...
    const verifierAcces = async (session) => {
      ...
      const participant = await resoudreParticipant(session);
      if (!actif) return;
      onDone({
  ...
  const demanderRecuperation = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
      redirectTo: "https://www.toolbox-pep.com/defi-strava/",
    });
    setEnvoi(false);
    if (error) {
    ...
    return (
      <Cadre>
        <div style={{ textAlign: "center", fontSize: 14.5, color: "#495260", lineHeight: 1.6, marginBottom: 18 }}>
          Ton compte existe, mais tu n'as pas accès au Défi Strava.
