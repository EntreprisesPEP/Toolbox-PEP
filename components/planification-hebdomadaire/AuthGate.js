...
// Client sur le schema planif_hebdo (profils)
const supabasePH = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { db: { schema: "planif_hebdo" } });

const LOGO_PEP = "/_static/planification-hebdomadaire/logo-pep.png";

/* ---------------------------------------------------------------------
   AUTH GATE — Planification hebdomadaire
   Login obligatoire des l'arrivee (contrairement a Ordre du jour, ici
   il n'y a pas de mode "lecture libre sans compte"). L'acces est
   reserve aux personnes cochees "Planification hebdomadaire" dans le
   panneau /administration/ du Toolbox (typiquement les gens de bureau
   — pas les contremaitres ni les dispatchs).

   Le mode "animateur / participant" (mot de passe partage, voir
   Header.js / check-password.js) reste totalement separe de ceci —
   il gere seulement qui peut EDITER une fois deja connecte, pas qui
   peut entrer.
--------------------------------------------------------------------- */
      ...
      <div style={{ width: "100%", maxWidth: 420, background: "#fff", border: "1px solid #D7DBE0", boxShadow: "0 1px 0 #D7DBE0" }}>
        <div style={{ background: "#0F2138", padding: "22px 24px", borderTop: "4px solid #E4022E", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <div style={{ color: "#B9C2CC", fontSize: 13, marginTop: 2 }}>Planification hebdomadaire</div>
...
export default function AuthGate({ onDone }) {
  // phase: 'verification' | 'login' | 'acces-refuse' | 'sans-profil' | 'recuperation-demandee' | 'recuperation-nouveau-mdp'
  ...
  useEffect(() => {
    ...
    const verifierAcces = async (session) => {
      const { data: appAccess } = await supabase
        ...
        .eq("user_id", session.user.id).eq("app_slug", "planification-hebdomadaire").maybeSingle();
      ...
      const { data: profil, error } = await supabasePH
        .from("profils").select("*").eq("user_id", session.user.id).maybeSingle();
  ...
  const demanderRecuperation = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
      redirectTo: "https://www.toolbox-pep.com/planification-hebdomadaire/",
    });
    setEnvoi(false);
    if (error) {
    ...
    return (
      <Cadre>
        <div style={{ textAlign: "center", fontSize: 14.5, color: "#495260", lineHeight: 1.6, marginBottom: 18 }}>
          Ton compte existe, mais tu n'as pas accès à Planification hebdomadaire.
  ...
  if (phase === "sans-profil") {
    return (
      <Cadre>
        <div style={{ textAlign: "center", fontSize: 14.5, color: "#495260", lineHeight: 1.6, marginBottom: 18 }}>
          Ton compte existe, mais n'a pas encore de profil configuré pour
          Planification hebdomadaire. Contacte l'administrateur pour qu'il
          complète ton profil dans le panneau d'administration du Toolbox.
        </div>
        <button
          style={BOUTON_PRINCIPAL}
          onClick={async () => { await supabase.auth.signOut(); setPhase("login"); }}
